...
public class LinkedList : IEnumerable<int>
{
    ...
    public void Remove(int value)
    {
        ...
        while (curr is not null)
        {
            if (curr.Data == value)
            {
                ...
                // then we can call "RemoveHead" to remove the value at the start.
                if (curr == _head)
                {
                    RemoveHead();
                    return; // Exit the function after removal is done
                }

                // If the location of 'value' is at the end of the list,
                // then we can call "RemoveTail" to remove the value at the end.
                if (curr == _tail)
                {
                    RemoveTail();
                    return; // Exit the function after removal is done
                }
                // For any other location of 'value', need to disconnect links from current
                // Next and Prev nodes, delete the current node, then reconnect the Next and Prev
                // nodes to each other
                else
                {
                    
                    curr.Prev!.Next = curr.Next; // connect previous node to the node after curr
                    curr.Next!.Prev = curr.Prev; // Connect next node to the node before curr
                }
    ...
    public void Replace(int oldValue, int newValue)
    {
        ...
        while (curr is not null)
        {
            ...
            {
                // if the location of 'oldValue' is found
                // set value at that location to 'newValue'
                curr.Data = newValue;
            }
    ...
    {
        // TODO Problem 5
        var curr = _tail; // Start at the ending since this is a reverse iteration.
        while (curr is not null)
        {
            yield return curr.Data; // Provide (yield) each item to the user
